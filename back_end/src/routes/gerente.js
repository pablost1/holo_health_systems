...
router.post('/nova_reserva',login_gerente,(req,res)=>{
    pool.getConnection((err,conn)=>{
        ...
        conn.query("SELECT * FROM sala WHERE id_sala=?",[req.body.id_sala],(err,results)=>{
            ...
            conn.query("SELECT * FROM reserva WHERE data=? AND (hor_ini<=? OR hor_fin>=?) AND id_sala=?",[req.body.data,req.body.hor_fin,req.body.hor_ini,req.body.id_sala],(err,results)=>{
                if(err){return res.status(500).send({error:err})}
                ...
                conn.query("SELECT * FROM medico_consultorio WHERE id_medico=? AND id_consultorio",[req.body.id_medico,req.usuario.id_consultorio],(err,results)=>{
                    ...
                    conn.query("SELECT * FROM reserva WHERE data=? AND (hor_ini>? OR hor_fin<?) AND id_medico=?",[req.body.data,req.body.hor_fin,req.body.hor_ini,req.body.id_medico],(err,results)=>{
                        if(err){return res.status(500).send({error:err})}
